...
class MoveInCircleComponent(owner: Mesh) extends Component(owner) with Update {
  ...
  def update(): Unit = {
    ...
    val newPosX = oldPosition.x + (4 * (Math.sin(thisUpdateTime) - Math.sin(lastUpdateTime))).toFloat
    val newPosY = oldPosition.y + (2 * (Math.cos(thisUpdateTime) - Math.cos(lastUpdateTime))).toFloat
    val newPosZ = oldPosition.z + (2 * (Math.cos(thisUpdateTime) - Math.cos(lastUpdateTime))).toFloat
